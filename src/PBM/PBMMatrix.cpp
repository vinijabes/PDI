...
namespace PDI {
	...
	Matrix<int> PBMMatrix::Load(const std::string& path)
	{
	...
	void PBMMatrix::Save(const std::string& path, const Matrix<int>& matrix)
	{
		std::ofstream file(path, std::ofstream::out);
		std::string line;

		WriteHeader(file, PBMFormat::P2, matrix);
		auto writer = PBMWriter(file);
		PGM::Save(matrix, writer);
	}
